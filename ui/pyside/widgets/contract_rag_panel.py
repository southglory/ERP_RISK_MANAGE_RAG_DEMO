"""계약·판례 RAG 패널 — Solar LLM + pgvector + bge-reranker."""
...
_SAMPLE_QUERIES = [
    "계약 해지 시 위약금 산정 기준 — 판례상 인정되는 손해배상 범위는?",
    "소프트웨어 유지보수 계약에서 SLA 위반 패널티 조항의 법적 효력은?",
    "하도급 계약의 대금 지연 시 지연이자 및 지급보증 의무 기준은?",
]


class ContractRAGPanel(QWidget):
    """계약·판례 RAG 패널."""
    ...
    def _build_ui(self) -> None:
        ...
        title = QLabel("📜 계약·판례 RAG — Solar LLM")
        ...
        for st, label in _SOURCE_LABELS.items():
            cb = QCheckBox(label)
            if st in (SourceType.CONTRACT, SourceType.COURT):
                cb.setChecked(True)
